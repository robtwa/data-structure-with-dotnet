...
public class ContainerWithMostWater
{
    ...
    {
        // two pointers
        // start: left boundary, end: right boundary
        int start = 0;
        int end = height.Length - 1;
        int max = 0;

        // Loop while the two pointers do not meet
        while (start < end)
        {
            ...
            {
                high = height[start];
                start++;  // Left side is shorter → move the left pointer
            }
            else
            {
                high = height[end];
                end--;    // Right side is shorter → move the right pointer
            }
